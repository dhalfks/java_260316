...
public class Static01 {
	...
	public static void main(String[] args) {
		/* static : 전역변수 / 전역메서드 (클래스변수 / 클래스메서드)
		 * 전 모든 객체에 공통으로 사용이 가능. 
		 * 프로그램이 실행되면서 바로 메모리에 올라가서 ~ 프로그램이 종료될때 같이 소멸
		 * - 객체 생성없이 클래스가 만들어지면서 생성
		 * - 객체 생성없이 클래스 만으로 호출 가능
		 * - 클래스명.변수명 / 클래스명.메서드명 => 사용방법
		 * - 객체 생성 후 사용해도 무관하나, 굳이 그렇게 사용하지 않음. (노란밑줄)
		 * - 클래스의 멤버변수, 메서는 하나의 멤버변수가 모든 객체에 공유되어야 할 때 사용
		 * 
		 ...
		 * - 각 객체마다 독립적으로 존재
		 * 
		 * 클래스의 멤버와, 객체의 멤버는 생성되는 시점이 달라서 메서드와 멤버변수의 사용
		 * 시점이 다르다. 
		 ...
		 *  - static이 붙은 메서드에서 일반 메서드를 호출 불가능. (객체 생성 후 사용가능)
		 * */
...
class TV{
	...
	public static void printBrand2() {
		...
		System.out.println("Brand : "+BRAND); // static
		//System.out.println(power); // 일반 멤버변수 => 호출 불가능
	}
