...
public class Exception01 {
	...
	public static void main(String[] args) {
		 ...
		 * - Error : 개발자가 복구할 수 없는 심각한 오류 (코드에 문제가 아닌 외부 요인이 원인)
		 * - Exception : 개발자의 코드에 의해 실수가 발생되는 경우 (개발자가 코드를 수정가능)
		 *   
		 *   - Checked Exception : 컴파일 시점에서 예외처리가 일어나는 예외
		 *   	=> IOException / SQLException / FileNotFoundException ...
		 *   
		 *   - UnChecked Exception : 문법적인 오류는 없으나(컴파일 시점에서는 문제가 되지않았음)
		 *     실행 시 발생되는 예외
		...
		try {
			System.out.println("---try 안영역");
			System.out.println(a+b);
			System.out.println(a-b);
			System.out.println(a*b);
			System.out.println("---try 안영역 예외발생시점");
			System.out.println(a/b); // exception 발생 시점
			System.out.println(a%b); // exception 발생 시점
			
			// 출력 안됨.
			System.out.println("---try 안영역 예외발생시점 이후 영역");
			System.out.println(a+b);
			System.out.println(a-b);
			System.out.println(a*b);
